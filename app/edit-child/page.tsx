...
export default function AddChildPage() {
  ...
  useEffect(() => {
    ...
    const loadChildData = async () => {
      ...
      try {
        const response = await fetch(`/api/children?icd=${encodeURIComponent(originalIcdCode)}`, {
          cache: "no-store",
        });
        ...
        if (!response.ok) {
          throw new Error(data.message || "Failed to load child profile.");
        }
        if (isCancelled || !data.child) {
          return;
        }
        const fullContact = (data.child.parentContact || "").trim();
        const knownCountryCodes = ["+91", "+1", "+44", "+61"];
        const matchedCountryCode =
          knownCountryCodes.find((code) => fullContact.startsWith(code)) || "+91";
        setCountryCode(matchedCountryCode);
        setParentContact(fullContact.slice(matchedCountryCode.length));
        setChildName(data.child.childName || "");
        setDateOfBirth(data.child.dateOfBirth || "");
        setDiagnosis(data.child.diagnosis || "");
        setTherapistId(data.child.assignedTherapistId || "");
        setIcdCode(data.child.icdCode || "");
        setInsurance(data.child.insurance || "");
        setIntakeNotes(data.child.intakeNotes || "");
      } catch (error) {
        if (!isCancelled) {
          showTimedFeedback({
            kind: "error",
            text: error instanceof Error ? error.message : "Failed to load child profile.",
          });
        }
      } finally {
        if (!isCancelled) {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      showTimedFeedback({
        kind: "error",
        text: data.message || "Unable to edit child profile.",
      });
    } catch {
      showTimedFeedback({
        kind: "error",
        text: "Unable to edit child profile.",
      });
    } finally {
  ...
  return (
    <main className="relative h-screen w-screen bg-white">
      ...
      >
        {isSubmittingChild ? "Editing..." : "Edit"}
      </button>
      {feedback ? (
        <p
          className={`${manrope.className} absolute m-0`}
      ...
      {showEditConfirmDialog ? (
        ...
        >
          ...
          >
            ...
            >
              Confirm to apply the changes to this child profile.
            </p>
      ...
      {isLoadingChildData ? (
        <div
          className={`${manrope.className} fixed inset-0 z-[1100] flex items-center justify-center bg-[#FFFFFFCC]`}
          style={{
            color: "#121712",
            fontWeight: 500,
            fontSize: "14px",
