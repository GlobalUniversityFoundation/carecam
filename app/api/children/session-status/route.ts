...
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as { icd?: string; uploadEpoch?: string };
    ...
    const currentStatus = result.session.status?.trim() || "Awaiting";
    if (currentStatus === "Pending review") {
      ...
      return NextResponse.json({ status: "Reviewed" }, { status: 200 });
    }
